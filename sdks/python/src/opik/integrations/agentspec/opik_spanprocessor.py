...
from pyagentspec.tracing.events import Event, LlmGenerationResponse
...
class OpikSpanProcessor(SpanProcessor):
    ...
    def _create_opik_span_outputs_from_span(
        ...
        # We consider the serialization of the last event,
        # i.e., the span end event, as the input of the span
        if len(span.events) < 2:
            # We expect to have at least 2 events, we exclude the first as it is the start event
            return {}
        event_dump = span.events[-1].model_dump(
            mask_sensitive_information=self.mask_sensitive_information
        )
    ...
    def _create_opik_span_usage_from_span(
        ...
        if isinstance(span, LlmGenerationSpan) and len(span.events) >= 2:
            ...
            event = span.events[-1]
            if isinstance(span, LlmGenerationResponse):
